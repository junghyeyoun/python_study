# Beatiful Soup : HTML 및 XML 파일에서 데이터를 가져오는 Python 라이브러리
...
def go():
    ...
    convert_data = BeautifulSoup(plain_text, 'lxml')  # parser : lxml
    print(type(convert_data)) # 형변환 -> <class 'bs4.BeautifulSoup'>
    
    for link in convert_data.findAll('a'):
        href = link.get('href')  #Building a clickable url
