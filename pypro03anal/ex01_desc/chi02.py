...
가설 검정은 수집된 데이터를 근거로 기존에 인식된 생각(귀무)을 기준으로 새롭게 제기된 주장을 대립가설로 만들어 검증하는 것이다.  
'''
# 귀무 : 기대치와 관찰치는 차이가 없다. 주사위는 게임에 적합하다.
# 대립 : 기대치와 관찰치는 차이가 있다. 주사위는 게임에 적합하지 않다.

# 실험에 의해 수집된 집단이 한 개이므로 일원카이제곱 검정 실시 
import pandas as pd 
import scipy.stats as stats

ob_sdata = [4,6,17,16,8,9]
result = stats.chisquare(ob_sdata) # (관찰값, 기대값, 자유도, 진행방향)
print(result) # statistic=14.200000000000001, pvalue=0.014387678176921308
print('검정 통계량 : %.5f, p-value:%.5f'%(result))
...
# 해석 : pvalue 0.00039 < 0.05 이므로 귀무기각. 대립 채택
# 스포츠 음료의 선호도에 차이가 있다는 결론을 얻을 수 있음. 그러므로 어떠한 조치(의사결정)를 하는데 있어 참고자료로 활용 가능 
