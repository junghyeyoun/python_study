...
# 두 개의 요인에 대한 집단(독립변수) 각각이 종속변수의 평균에 영향을 주는지 검정
...
print(data2.head(3), data2.shape) # 48행 4열

# 주효과 가설
# 귀무 : 독의 종류(poison)와 독퍼짐 시간의 평균은 차이가 없다.
# 대립 : 독의 종류(poison)와 독퍼짐 시간의 평균은 차이가 있다.
# 귀무 : 응급처치 방법(treat)과 독퍼짐 시간의 평균은 차이가 없다.
# 대립 : 응급처치 방법(treat)과 독퍼짐 시간의 평균은 차이가 있다.
# 교호작용 가설
# 귀무 : 교호작용이 없다.(독의 종류와 응급처치 방법은 관련이 없다 ->  서로 독립적이다.)
# 대립 : 교호작용이 있다.(독의 종류와 응급처치 방법은 관련이 있다 ->  서로 독립적이지 않다.)
...
# 모든 집단별 표본 수가 동일함으로 균형설계가 잘되었다라고 할 수 있다.
...
# C(poison):C(treat)   6.0  0.250138  0.041690   1.874333  1.122506e-01 > 0.05이므로 귀무가설 기각 실패-> 교호작용은 발견할 수 없다.