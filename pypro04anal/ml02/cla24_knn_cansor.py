...
x_train, x_test, y_train, y_test = train_test_split(cancer.data, cancer.target, random_state=66,stratify=cancer.target,test_size=0.2) 
# stratify= 속성을 사용하면 편향을 방지함. 분류의 경우에는 성능을 보장받기 위해 사용을 권장
...
# 최적의 k를 위한 시각화 -> 훈련데이터 정확도와 검증데이터 정확도 가장 가깝게 만나는 지점 찾기
