# 부스팅은 데이터셋 모델이 뒤의 데이터셋을 정해주고 앞의 모델을 보완해 나가며 학습시켜 준다. (순차, 직렬 학습)
...
from lightgbm import LGBMClassifier # xgboost 보다 연산량이 적어 손실을 줄일 수 있다. 대용량 처리가 효율적임.
...
print(np.sum(y_label == 1)) # benign:257 악성
...
# model = LGBMClassifier(boosting_type='gbdt').fit(x_train, y_train)  # plot_importance 지원안함
print(model)
