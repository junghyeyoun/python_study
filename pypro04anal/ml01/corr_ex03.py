# 공공데이터 중 출입국관광서비스 잘를 사용 - 국내 유료 관광지에 방문한 외국인 관광객 관련
import pandas as pd 
import numpy as np 
...
def makeScatterGraph(tour_table, all_table, tourpoint):
    ...
    plt.tight_layout() # plot간의 간격 맞춰줌
    plt.show()
...
def Chulbal():
    ...
    print('관광지명 : ', resNm)
    print('관광지명 : ', resNm[:5]) # 자료가 너무 많이 때문에 다섯개만 뽑아서 하기 -> ['창덕궁' '운현궁' '경복궁' '창경궁' '종묘']
