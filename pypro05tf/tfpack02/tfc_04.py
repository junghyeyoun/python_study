# 다항분류 : 출력값이 softmax로 인해 확률 값으로 여러 개가 출력(범주형으로 처리 - 가장 큰 인덱스를 취함)
# label은 one-hot 인코딩
...
# dataset 작성
xdata = np.random.random((1000, 12))  # 천행 12열, 임의의 과목에 대한 시험점수라 가정
...
print(ydata[:2])  # label
print([np.argmax(i) for i in ydata[:2]])  # index값 중에서 뭐가 가장 큰것을 찾음
...
print('분류 결과 : ', new_pred)  # 확률값으로 나올거임
